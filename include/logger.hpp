...
public:
    ...
    void export_csv_file(const std::filesystem::path& path) const;
    ...
    // Latched after start_session(); empty when no session is active. Used
    // by the GUI's web-monitor page so the live snapshot session id matches
    // the on-disk CSV/JSON filename.
...
private:
    ...
    // Serializes every public entry point: writes from the worker thread
    // (log_temperature / log_params / log_event / log_error / set_* /
    // start_session / end_session) and the GUI thread's flush + export_csv +
    // error_summary_text + error_count. Recursive so internal call chains
    // (start_session → log_event → write_row → maybe_flush → flush) stay
    // simple. Mutable so const accessors (export_csv_file, error_count,
    // error_summary_text) can also lock.
